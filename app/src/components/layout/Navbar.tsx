...
export function Navbar() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("cg-lang") as "FR" | "EN" | null;
  ...
  const toggleLang = () => {
    ...
    setLang(next);
    localStorage.setItem("cg-lang", next);
  };
    ...
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3.5">
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          <button
            onClick={toggleLang}
            className="flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground hover:border-foreground/30"
          ...
          <Link
            href="/onboarding"
            className="ml-2 inline-flex items-center rounded-lg bg-primary px-5 py-2 text-sm font-medium text-white transition-all hover:bg-primary-hover hover:shadow-md"
          >
            Commencer
          </Link>
